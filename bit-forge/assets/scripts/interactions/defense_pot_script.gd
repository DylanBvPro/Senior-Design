...
@export var defense_duration_seconds: float = 60.0
@export var incoming_damage_multiplier: float = 0.5
...
@export var pickup_shape_path: NodePath = NodePath("bottle_C_brown/CollisionShape3D2")
...
func _try_apply_pickup(body: Node) -> void:
	if body == null or not is_instance_valid(body):
		return
	if not body.has_method("apply_temporary_defense_bonus"):
		return

	_picked_up = true
	body.call("apply_temporary_defense_bonus", defense_duration_seconds, incoming_damage_multiplier)
	queue_free()
