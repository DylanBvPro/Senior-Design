...
@export_group("Boss Taunt")
@export var taunt_animations: Array[StringName] = []
@export var taunt_min_duration_seconds: float = 1.0
@export var auto_taunt_enabled: bool = true
@export var taunt_cooldown_seconds: float = 8.0
@export var use_random_taunt_interval: bool = false
@export var taunt_interval_min_seconds: float = 6.0
@export var taunt_interval_max_seconds: float = 12.0
@export var taunt_activation_target_path: NodePath
@export var taunt_activation_method: StringName = "activate"
@export_group("Boss Enrage")
@export_range(0.01, 1.0, 0.01) var enrage_full_effect_health_ratio: float = 0.15
@export var max_speed_multiplier_at_low_health: float = 2.0
@export var min_taunt_cooldown_multiplier_at_low_health: float = 0.5
@export var min_attack_cooldown_multiplier_at_low_health: float = 0.5
@export_group("Boss Combat")
@export var max_player_knockback_multiplier_at_low_health: float = 1.35
...
var _taunt_serial: int = 0
var _next_allowed_taunt_time: float = 0.0
...
func activate_taunt() -> void:
	...
	if stun_time_remaining > 0.0:
		return
	if not _can_start_taunt_now():
		return

	_schedule_next_taunt_window()

	_start_taunt_sequence()


func _start_taunt_sequence() -> void:
	if not player_in_detection:
		return

	_taunt_serial += 1
	var taunt_serial: int = _taunt_serial
	is_taunting = true
	...
	_activate_taunt_target()

	var chosen_taunt: StringName = _choose_taunt_animation()
	var taunt_duration: float = maxf(taunt_min_duration_seconds, 0.1)
	if animation_player and chosen_taunt != StringName("") and animation_player.has_animation(chosen_taunt):
		animation_player.play(chosen_taunt)
		var taunt_anim: Animation = animation_player.get_animation(chosen_taunt)
		if taunt_anim:
			taunt_duration = maxf(taunt_duration, taunt_anim.length)

	await get_tree().create_timer(taunt_duration).timeout
	if taunt_serial != _taunt_serial:
		return
	if is_dead:
		return

	is_taunting = false
	if target != null and is_instance_valid(target) and player_in_detection:
		state_chart.send_event("toFollow")
	else:
		state_chart.send_event("toIdle")


func _can_start_taunt_now() -> bool:
	var now_seconds := float(Time.get_ticks_msec()) / 1000.0
	return now_seconds >= _next_allowed_taunt_time


func _schedule_next_taunt_window() -> void:
	var now_seconds := float(Time.get_ticks_msec()) / 1000.0
	var delay_seconds: float = maxf(taunt_cooldown_seconds, 0.0)
	if use_random_taunt_interval:
		var min_delay: float = maxf(taunt_interval_min_seconds, 0.0)
		var max_delay: float = maxf(taunt_interval_max_seconds, min_delay)
		delay_seconds = randf_range(min_delay, max_delay)

	# As health drops, taunts become more frequent.
	delay_seconds *= _get_taunt_cooldown_multiplier()

	_next_allowed_taunt_time = now_seconds + delay_seconds


func _activate_taunt_target() -> void:
	if taunt_activation_target_path == NodePath(""):
		return
	if taunt_activation_method == StringName(""):
		return

	var target_node: Node = get_node_or_null(taunt_activation_target_path)
	if target_node == null:
		return
	if not target_node.has_method(taunt_activation_method):
		return

	target_node.call(taunt_activation_method)


func _choose_taunt_animation() -> StringName:
	if animation_player == null:
		return StringName("")

	var valid_taunts: Array[StringName] = []
	for anim_name in taunt_animations:
		if animation_player.has_animation(anim_name):
			valid_taunts.append(anim_name)

	if valid_taunts.is_empty():
		return StringName("")
	if valid_taunts.size() == 1:
		return valid_taunts[0]

	return valid_taunts[randi() % valid_taunts.size()]
...
func _on_follow_state_physics_processing(delta: float) -> void:
	...
	if distance_to_target < attack_range and player_in_detection and time_since_last_attack >= _get_current_attack_cooldown():
	...
	if direction.length() > 0.01:
		...
		nav_agent.velocity = direction * (enemy_info.follow_speed * speed_multiplier)
...
func _on_attack_state_physics_processing(delta: float) -> void:
	...
	if enemy_info.attack_type == EnemyInfo.AttackType.RANGED:
		...
		if time_since_last_attack < _get_current_attack_cooldown():
			return
...
func _update_ranged_attack_placeholder(_delta: float) -> void:
	...
	nav_agent.velocity = Vector3.ZERO
	var speed_multiplier: float = _get_speed_multiplier()
	velocity.x = retreat_dir.x * (enemy_info.ranged_retreat_speed * speed_multiplier)
	velocity.z = retreat_dir.z * (enemy_info.ranged_retreat_speed * speed_multiplier)
	is_ranged_backpedaling = true


func _get_speed_multiplier() -> float:
	var t: float = _get_enrage_progress()
	var max_speed: float = maxf(max_speed_multiplier_at_low_health, 1.0)
	return lerpf(1.0, max_speed, t)


func _get_taunt_cooldown_multiplier() -> float:
	var t: float = _get_enrage_progress()
	var min_cooldown_mult: float = clampf(min_taunt_cooldown_multiplier_at_low_health, 0.01, 1.0)
	return lerpf(1.0, min_cooldown_mult, t)
...
func _get_current_attack_cooldown() -> float:
	...
	var t: float = _get_enrage_progress()
	var min_cooldown_mult: float = clampf(min_attack_cooldown_multiplier_at_low_health, 0.01, 1.0)
	var cooldown_multiplier: float = lerpf(1.0, min_cooldown_mult, t)
	return maxf(enemy_info.attack_cooldown * cooldown_multiplier, 0.01)


func _get_enrage_progress() -> float:
	if enemy_info == null:
		return 0.0

	var max_health_value: float = maxf(enemy_info.max_health, 0.001)
	var health_ratio: float = clampf(current_health / max_health_value, 0.0, 1.0)
	var threshold: float = clampf(enrage_full_effect_health_ratio, 0.01, 1.0)

	# 0.0 at full health, 1.0 when health is at or below the threshold.
	return clampf((1.0 - health_ratio) / (1.0 - threshold), 0.0, 1.0)


func get_player_knockback_multiplier() -> float:
	var t: float = _get_enrage_progress()
	var max_knockback_mult: float = maxf(max_player_knockback_multiplier_at_low_health, 1.0)
	return lerpf(1.0, max_knockback_mult, t)
